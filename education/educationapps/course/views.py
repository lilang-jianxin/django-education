...
class CommentsView(LoginRequiredMixin, View):
   def get(self, request, course_id):
       course = Course.objects.get(id=int(course_id))
       all_resources = CourseResource.objects.filter(course=course)
       all_comments = CourseComments.objects.all().filter(course=course).order_by("-id")
...
class AddComentsView(View):
    ...
    def post(self, request):
        if not request.user.is_authenticated():
            # 判断用户登录状态
            return HttpResponse('{"status":"failure", "msg":"用户未登录"}', content_type='application/json')
        ...
        if course_id > 0 and comments:
            ...
            return HttpResponse('{"status":"failure", "msg":"添加失败"}', content_type='application/json')